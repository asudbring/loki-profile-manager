...
func classifyCleanupChange(record ManagedTarget, keepTargets map[string]bool) (CleanupChange, bool, error) {
	...
	if record.Mode == string(OperationRender) {
		if info.IsDir() {
			change.Status = CleanupBlocked
			change.Message = "obsolete rendered target is a directory; remove or adopt it manually"
			return change, true, nil
		}
		change.Status = CleanupRemovable
		change.Message = "obsolete rendered target is generated by Loki"
		return change, true, nil
	}
